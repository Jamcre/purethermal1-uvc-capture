...
def display_temperature(img, val_k, loc, color):
    val = ktof(val_k)
    cv2.putText(img, "{0:.1f} degF".format(val), loc, cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 1)
    x, y = loc
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if thermal_data is not None:
            ...
            print(f"Clicked at ({x}, {y}): {ktof(last_click_temp):.1f}°F / {ktoc(last_click_temp):.1f}°C")
            ...
            try:
                while True:
                    ...
                    if last_click_pos is not None and (time.time() - last_click_time) < 3:
                        ...
                        last_click_pos = None  # Clear expired click
