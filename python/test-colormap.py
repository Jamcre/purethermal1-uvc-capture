...
# ====== NEW: Colormap Configuration ======
COLORMAPS = [
    ("TURBO", cv2.COLORMAP_TURBO),      # Default - best for thermal
    ("INFERNO", cv2.COLORMAP_INFERNO),  # High contrast
    ("JET", cv2.COLORMAP_JET),          # Traditional thermal
    ("HOT", cv2.COLORMAP_HOT),          # Emphasizes high temps
    ("GRAY", cv2.COLORMAP_BONE)         # Grayscale
]
current_colormap = cv2.COLORMAP_TURBO  # Initialize default
...
# ====== MODIFIED: Dynamic Colormap ======
def raw_to_8bit(data):
    cv2.normalize(data, data, 0, 65535, cv2.NORM_MINMAX)  # Normalize 16-bit to 0-65535
    np.right_shift(data, 8, data)  # Convert to 8-bit (0-255)
    data = np.uint8(data)
    data = cv2.applyColorMap(data, current_colormap)  # Use selected colormap
    return data
...
def create_colorbar(min_temp, max_temp, height=480, width=100):
    """Create colorbar using current colormap"""
    gradient = np.linspace(0, 255, height).astype(np.uint8)
    gradient = np.tile(gradient, (width, 1)).T
    colorbar = cv2.applyColorMap(gradient, current_colormap)  # Dynamic colormap
    
    # Labels and ticks (unchanged)
    ...
    try:
        ...
        try:
            ...
            cv2.setMouseCallback('Lepton Radiometry', mouse_callback)
              ...
              # ====== NEW: Trackbar Creation ======
            cv2.createTrackbar(
                "Colormap",                 # Name
                "Lepton Radiometry",        # Window
                0,                          # Default position (TURBO)
                len(COLORMAPS)-1,           # Max position
                lambda x: None              # No callback needed
            )
